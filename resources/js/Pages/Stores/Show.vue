        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
            ...
            <nav class="mb-8">
                <ol class="flex flex-wrap items-center gap-2 text-sm">
                    ...
                    <li>
                        <span class="text-gray-500 dark:text-gray-400">{{ t('nav.stores') }}</span>
                    </li>
                    <li class="text-gray-400">/</li>
                    <li class="text-gray-900 dark:text-white font-medium">{{ store.name }}</li>
                </ol>
            </nav>

            <!-- Store header -->
            <div class="rounded-2xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-8 mb-8">
                <div class="flex flex-col sm:flex-row sm:items-center gap-6">
                    <!-- Store icon -->
                    <div class="flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl bg-indigo-100 dark:bg-indigo-900/50">
                    ...
                    <div class="flex-1">
                        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ store.name }}</h1>
                        <p v-if="store.description" class="mt-2 text-gray-600 dark:text-gray-400">
                            {{ store.description }}
                        </p>
                        ...
                        <div class="mt-4 flex flex-wrap items-center gap-6">
                            ...
                            <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                                ...
                                <span class="font-semibold text-gray-900 dark:text-white">{{ store.total_products }}</span>
                                {{ t('stores.products') }}
                            </div>
                            ...
                            <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                                ...
                                <span class="font-semibold text-gray-900 dark:text-white">{{ store.total_reviews }}</span>
                                {{ t('stores.reviews') }}
                            </div>

                            <!-- Average rating -->
                            <div v-if="store.average_rating" class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                                <StarRating :rating="Math.round(store.average_rating)" size="sm" />
                                <span class="font-semibold text-gray-900 dark:text-white">{{ store.average_rating }}</span>
                                {{ t('stores.avg_rating') }}
                            </div>
                        </div>
                    </div>
            ...
            <div v-else class="text-center py-16 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
                ...
                <h3 class="mt-4 text-lg font-semibold text-gray-900 dark:text-white">{{ t('products.no_results') }}</h3>
            </div>
        </div>
    </component>
</template>
